...
const FN_WRAPPER_HEAD = 'return __njstrace__fnWrapper({ fnId: %s, name: %s, file: %s, startLine: %s, startColumn: %s }, (function () {';
...
Injector.prototype.injectTracing = function(filename, code, wrapFunctions, includeArguments, wrappedFile) {
	...
	var output = falafel(code, {ranges: true, locations: true, ecmaVersion: 10}, function processASTNode(node) {
		...
		if (name && node.body.type === Syntax.BlockStatement) { // Not supporting arrow functions with no body
			...
			const fnId = `${name}@${filename}::${startLine}`;
			const head = util.format(FN_WRAPPER_HEAD, JSON.stringify(fnId), JSON.stringify(name), JSON.stringify(filename), startLine, node.loc.start.column);
