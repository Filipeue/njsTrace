...
Injector.prototype.injectTracing = function(filename, code, wrapFunctions, includeArguments, wrappedFile) {
	...
	var output = falafel(code, {ranges: true, locations: true, ecmaVersion: 10}, function processASTNode(node) {
		...
		// We want to skip wrapper function
		if (wrappedFile && startLine === 0) {
			return;
		}
		...
		if (name && name !== 'constructor' && !node.generator && node.body.type === Syntax.BlockStatement) { // Not supporting arrow functions with no body
			if (sourceMapConsumer) {
				...
				if (resolvedDescriptor.source === null) {
					return;
				}
				startLine = resolvedDescriptor.line;
				startColumn = resolvedDescriptor.column;
				const sourceAbsolutePath = path.resolve(path.dirname(filename), resolvedDescriptor.source);
				filePath = path.relative(process.cwd(), sourceAbsolutePath);
			}

			self.njs.log('  Instrumenting ', name, 'line:', startLine, 'colum:', startColumn);
			...
			const fnId = `${name}@${filePath}::${startLine}:${startColumn}`;
			const fnDeclaration = node.async ? 'async function ()' : node.generator ? 'function* ()' : 'function ()';
			const head = util.format(FN_WRAPPER_HEAD, JSON.stringify(fnId), JSON.stringify(name), JSON.stringify(filePath), startLine, startColumn, fnDeclaration);
